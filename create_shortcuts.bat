...
title Create Office2PDF Shortcuts

set "SCRIPT_DIR=%~dp0"
set "APP_EXE="
set "APP_DIR="

rem Preferred: this script is beside the completed release executable.
if exist "%SCRIPT_DIR%Office2PDF.exe" if exist "%SCRIPT_DIR%_internal\python311.dll" (
    set "APP_EXE=%SCRIPT_DIR%Office2PDF.exe"
    set "APP_DIR=%SCRIPT_DIR:~0,-1%"
)

rem Also support execution from the project root.
if not defined APP_EXE if exist "%SCRIPT_DIR%dist\Office2PDF\Office2PDF.exe" if exist "%SCRIPT_DIR%dist\Office2PDF\_internal\python311.dll" (
...
if not defined APP_EXE (
    ...
    echo.
    echo Do not use build\Office2PDF\Office2PDF.exe.
    echo.
    pause
    exit /b 1
)
...
if not exist "%CSCRIPT%" (
    echo ERROR: Windows Script Host was not found at:
    echo   %CSCRIPT%
    pause
    exit /b 2
)

set "VBS_FILE=%TEMP%\Office2PDF_CreateShortcuts_%RANDOM%_%RANDOM%.vbs"
...
>>"%VBS_FILE%" echo WScript.Echo "Created Desktop shortcut:" ^& vbCrLf ^& desktopLink
>>"%VBS_FILE%" echo WScript.Echo "Created Start Menu shortcut:" ^& vbCrLf ^& startMenuLink
...
echo   %APP_EXE%
echo.
echo Creating Office2PDF shortcuts...
...
if not "%RESULT%"=="0" (
    echo.
    echo ERROR: Windows could not create the shortcuts.
...
echo.
echo SUCCESS: Office2PDF shortcuts were created.
echo.
pause
exit /b 0
